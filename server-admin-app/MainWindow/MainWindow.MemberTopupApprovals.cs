...
{
    private readonly HashSet<string> _memberTopupHandledRequestIds = new(StringComparer.OrdinalIgnoreCase);
    private bool _isLoadingPendingMemberTopupRequests;

    private void HandleRealtimeMemberTopupRequested(MemberTopupRequestItem? request)
    {
        ...
        if (!_memberTopupHandledRequestIds.Add(request.RequestId))
        {
            return;
        }
    ...
    private async Task LoadPendingMemberTopupRequestsAsync()
    {
        if (_isLoadingPendingMemberTopupRequests)
        {
            return;
        }

        _isLoadingPendingMemberTopupRequests = true;
        try
        {
            var response = await _httpClient.GetFromJsonAsync<MemberTopupPendingListResponse>(
                BuildApiUrl("/members/topup-requests/pending"),
                JsonOptions());
            ...
            {
                HandleRealtimeMemberTopupRequested(item);
            }
        }
        catch (Exception ex)
        {
            AppendServiceLog($"[{DateTime.Now:HH:mm:ss}] Không tải được danh sách chờ duyệt nạp tiền: {ex.Message}");
        }
        finally
        {
            _isLoadingPendingMemberTopupRequests = false;
        }
    }
    ...
    private async Task ShowMemberTopupApprovalDialogAsync(MemberTopupRequestItem request)
    {
        ...
        await Dispatcher.InvokeAsync(() =>
        {
            ...
            {
                Title = "Yêu cầu nạp tiền hội viên",
                Width = 480,
                Height = 340,
            ...
            {
                Text = "Hội viên gửi yêu cầu nạp tiền",
                FontSize = 18,
            ...
            {
                Text = $"Số tiền nạp: {request.Amount:N0} VND",
                Margin = new Thickness(0, 0, 0, 6),
                FontWeight = FontWeights.SemiBold,
            ...
            rejectButton.Click += async (_, _) =>
            {
                ...
                try
                {
                    using var response = await _httpClient.PostAsJsonAsync(
                        BuildApiUrl($"/members/topup-requests/{request.RequestId}/reject"),
                        new
                        {
                            rejectedBy = "admin.desktop",
                            note = "Admin hủy yêu cầu nạp tiền",
                        });
                    ...
                    AppendServiceLog(
                        $"[{DateTime.Now:HH:mm:ss}] Đã hủy yêu cầu nạp tiền {request.RequestId} của {memberName}: {request.Amount:N0} VND");
            ...
            approveButton.Click += async (_, _) =>
            {
                ...
                {
                    using var response = await _httpClient.PostAsJsonAsync(
                        BuildApiUrl($"/members/topup-requests/{request.RequestId}/approve"),
                        new
                        {
                    ...
                    AppendServiceLog(
                        $"[{DateTime.Now:HH:mm:ss}] Đã chấp nhận nạp tiền {request.RequestId} của {memberName}: {request.Amount:N0} VND");
